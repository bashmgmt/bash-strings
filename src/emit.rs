...
pub fn emit_scalar(s: &str) -> String {
    ...
    if needs_ansi_c(s) { return ansi_c(s); }
...
mod tests {
    ...
    fn q_words_emit() {
        ...
        assert_eq!(emit_q_words(&["a".into(), "b c".into()]),          "'a' 'b c'");
    }

    #[test]
    fn indexed_emit() {
        assert_eq!(emit_indexed(&ix([])),                                "()");
        assert_eq!(emit_indexed(&ix([(0, "a"), (5, "b c")])),            "([0]='a' [5]='b c')");
    }

    #[test]
    fn assoc_emit() {
        assert_eq!(emit_assoc(&ax([])),                                  "()");
        assert_eq!(emit_assoc(&ax([("k", "v"), ("k 2", "v 2")])),        "(['k']='v' ['k 2']='v 2')");
    }

    #[test]
    fn roundtrip_scalar() {
        for s in ["", "simple", "it's", "a\nb", "a\tb", "c\x01d", "café"] {
            assert_eq!(parse_scalar(&emit_scalar(s)).unwrap(), s);
        }
    }

    #[test]
    fn roundtrip_q_words() {
        let v = vec!["a".to_string(), "b c".to_string(), "d\ne".to_string(), "".to_string()];
        assert_eq!(parse_q_words(&emit_q_words(&v)).unwrap(), v);
    }

    #[test]
    fn roundtrip_indexed() {
        let m = ix([(0, "a"), (2, "d\ne"), (5, "b c")]);
        assert_eq!(parse_indexed(&emit_indexed(&m)).unwrap(), m);
    }
